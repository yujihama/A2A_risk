...
$projectRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent (Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $scriptDir)))))
...
foreach ($config in $configs) {
    ...
    $pythonCommand = "cd '$projectRoot'; python -m A2A_risk.samples.python.agents.data_agent --config '$fullConfigPath'"
