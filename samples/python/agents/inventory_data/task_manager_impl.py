...
# agent.py から run_agent をインポート
from .agent import run_agent

logger = logging.getLogger(__name__)

class InventoryDataTaskManager(InMemoryTaskManager):
    """
    InventoryDataAgent固有のタスク処理ロジックを実装したTaskManager。
    ...
    async def on_send_task_subscribe(
        ...
        logger.warning("ストリーミングタスク送信（on_send_task_subscribe）はこのプロトタイプでは実装されていません。")
        return new_not_implemented_error(request.id)

    async def on_resubscribe_to_task(
        self, request: TaskResubscriptionRequest
    ) -> Union[AsyncIterable[SendTaskStreamingResponse], JSONRPCResponse]:
        logger.warning("タスク再サブスクリプション（on_resubscribe_to_task）はこのプロトタイプでは実装されていません。")
        return new_not_implemented_error(request.id) 